...
void TaskItemWidget::setupUi() {
  ...
  prefixFont.setPointSize(9);

  // 计划时间前缀 badge
  mScheduledPrefixPtr = new QLabel(QStringLiteral("计划时间"), this);
  mScheduledPrefixPtr->setFont(prefixFont);
  mScheduledPrefixPtr->setStyleSheet(
      sDarkTheme
          ? QStringLiteral("QLabel{background:#3A3A50;color:#A0A0A0;border:1px "
                           "solid #505068;border-radius:4px;padding:1px 4px;}")
          : QStringLiteral(
                "QLabel{background:#E8E8E8;color:#666666;border:1px solid "
                "#D0D0D0;border-radius:4px;padding:1px 4px;}"));
  ...
  mActualPrefixPtr->setStyleSheet(
      sDarkTheme
          ? QStringLiteral("QLabel{background:#1B3A2A;color:#4CAF50;border:1px "
                           "solid #2E5A3A;border-radius:4px;padding:1px 4px;}")
          : QStringLiteral(
                "QLabel{background:#E8F5E9;color:#2E7D32;border:1px solid "
                "#A5D6A7;border-radius:4px;padding:1px 4px;}"));
